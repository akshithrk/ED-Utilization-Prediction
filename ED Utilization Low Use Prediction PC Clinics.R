...
# 2017 ED Utilization
EDLowusePC_cy2016.df <- read.xlsx("High ED Use Low Use in 2016 Primary Care Clinic Well Child 2015-2016.xlsx")

# Validation data set
EDLowusePC_cy2017.df <- read.xlsx("High ED Use Low Use in 2017 Primary Care Clinic Well Child 2016-2017.xlsx")

# Fit model

## Select from the full dataset just the variables to try fitting.

EDPredictorsLowusePC.2016.df <- select(EDLowusePC_cy2016.df, High.ED.Utilization.3.Indicator, Age.Group:Last.Primary.Care.Specialty.Seen,Prior.ED.Visit.Count.Orig, Gender, Major.Region, Race.and.Ethnicity.Group, ED.Shift)

## Fit a logistic regression model with all of the initial set of selected variables
EDPredictorsLowusePC.2016.logis <- glm(High.ED.Utilization.3.Indicator ~ ., family=binomial(logit), data=EDPredictorsLowusePC.2016.df)
...
# save the new model
save(EDPredictorsLowusePC.2016.logis.steplogis, file="EDPredictorsLowusePC.2016.logis.steplogis.Rda")

EDPredictorsLowusePC.2017.df <- select(EDLowusePC_cy2017.df, High.ED.Utilization.3.Indicator, Age.Group:Last.Primary.Care.Specialty.Seen,Prior.ED.Visit.Count.Orig, Gender, Major.Region, Race.and.Ethnicity.Group, ED.Shift)
# save the 2017 validation data set
save(EDPredictorsLowusePC.2017.df, file="EDPredictorsLowusePC2017.df.Rda")
...
evaluation.scores <- prediction(EDPredictorsLowusePC.t2016.v2017.steplogis.predict$fit, EDPredictorsLowusePC.2017.df$High.ED.Utilization.3.Indicator)
...
title( main="ROC Curve for Predictions of High ED Use with Low Resource Use in Primary Care Well Child Patients",
...
title(main="True Positive and True Negative Rate Plots by Cutoff\nFor Predictions of High ED Use with Low Resource Use in Primary Care Well Child Patients",
      sub="Logistic model trained with CY2016 and validated with 2017 data")
