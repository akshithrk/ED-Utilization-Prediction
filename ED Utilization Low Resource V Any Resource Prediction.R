...
# 2016 ED Utilization
EDLowResourceusePC_cy2016.df <- read.xlsx("Low Use in 2016 Primary Care Clinic Well Child 2015-2016.xlsx")

# Validation data set
EDLowResourceusePC_cy2017.df <- read.xlsx("Low Use in 2017 Primary Care Clinic Well Child 2016-2017.xlsx")

# Fit model

## Select from the full dataset just the variables to try fitting.

EDPredictorsLowResourceusePC.2016.df <- select(EDLowResourceusePC_cy2016.df, No.Resource.Use.Indicator, Age.Group:Last.Primary.Care.Specialty.Seen,Prior.ED.Visit.Count, Gender, Major.Region, Race.and.Ethnicity.Group, ED.Shift)

## Fit a logistic regression model with all of the initial set of selected variables
EDPredictorsLowResourceusePC.2016.logis <- glm(No.Resource.Use.Indicator ~ ., family=binomial(logit), data=EDPredictorsLowResourceusePC.2016.df)
summary(EDPredictorsLowResourceusePC.2016.logis)
...
EDPredictorsLowResourceusePC.2016.logis.steplogis <- stepAIC(EDPredictorsLowResourceusePC.2016.logis, trace=0)
summary(EDPredictorsLowResourceusePC.2016.logis.steplogis)
# save the new model
#save(EDPredictorsLowResourceusePC.2016.logis.steplogis, file="EDPredictorsLowResourcePC.2016.logis.steplogis.Rda")

EDPredictorsLowResourceusePC.2017.df <- select(EDLowResourceusePC_cy2017.df, No.Resource.Use.Indicator, Age.Group:Last.Primary.Care.Specialty.Seen,Prior.ED.Visit.Count, Gender, Major.Region, Race.and.Ethnicity.Group, ED.Shift)
# save the 2017 validation data set
# save(ICUPredictorsLowResourceusePC.JanDec2017.df, file="ICUPredictorsLowResourcePC.JanDec2017.df.Rda")

## Predict outcomes by feeding the validation predictor variables into the model
EDPredictorsLowResourceusePC.t2016.v2017.steplogis.predict <- predict(EDPredictorsLowResourceusePC.2016.logis.steplogis, newdata=EDPredictorsLowResourceusePC.2017.df, type="response", se.fit=TRUE)

## Tally up the predicted outcomes vs. the actual outcomes included in the validation dataset (This is called scoring.)

evaluation.scores <- prediction(EDPredictorsLowResourceusePC.t2016.v2017.steplogis.predict$fit, EDPredictorsLowResourceusePC.2017.df$No.Resource.Use.Indicator)
...
title( main="ROC Curve for Predictions of ED Use with Low Resource Use in Primary Care Well Child Patients",
...
title(main="True Positive and True Negative Rate Plots by Cutoff\nFor Predictions of ED Use with Low Resource Use in Primary Care Well Child Patients",
      sub="Logistic model trained with CY2016 and validated with 2017 data")
