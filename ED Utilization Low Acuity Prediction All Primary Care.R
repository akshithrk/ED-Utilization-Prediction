...
# 2016 ED Utilization
EDLowAcuityPC_cy2016.df <- read.xlsx("High ED Use Low Acuity in 2016 Primary Care Well Child 2015-2016.xlsx")

# Validation data set
EDLowAcuityPC_cy2017.df <- read.xlsx("High ED Use Low Acuity in 2017 Primary Care Well Child 2016-2017.xlsx")

# manual test to find error, was CCC GI, CC Respiratory, CCC Malignancy
# error: Warning message:
# glm.fit: fitted probabilities numerically 0 or 1 occurred

#EDPredictorsLowAcuityPC.2016.logis <- glm(High.ED.Utilization.Indicator ~ Age.Group+Problem.Count+CCC.Count
#                                        +CCC.Hem.Imm+CCC.Metabolic+CCC.Neuromuscular+CCC.Renal+CCC.Congenital+CCC.CVD+CCC.Metabolic
#                                        +CCC.Neonatal+Public.or.Private.Metric+Holiday.Indicator+ED.Checkin.Hour.Metric+ED.Checkin.Day.of.the.Week                                        
...
#                                        +Gender+Race.and.Ethnicity.Group+ED.Shift+Prior.ED.Visit.Count+Major.Region+ED.Triage.Acuity.Metric, family=binomial(logit), data=EDPredictorsLowAcuityPC.2016.df)
# summary(EDPredictorsLowAcuityPC.2016.logis)

# Fit model

## Select from the full dataset just the variables to try fitting.

EDPredictorsLowAcuityPC.2016.df <- select(EDLowAcuityPC_cy2016.df, High.ED.Utilization.Indicator, Age.Group:Prior.ED.Visit.Count, Gender, Race.and.Ethnicity.Group, ED.Shift, Season, Major.Region)

## Fit a logistic regression model with all of the initial set of selected variables
EDPredictorsLowAcuityPC.2016.logis <- glm(High.ED.Utilization.Indicator ~ ., family=binomial(logit), data=EDPredictorsLowAcuityPC.2016.df)
summary(EDPredictorsLowAcuityPC.2016.logis)
...
EDPredictorsLowAcuityPC.2016.logis.steplogis <- stepAIC(EDPredictorsLowAcuityPC.2016.logis, trace=0)
summary(EDPredictorsLowAcuityPC.2016.logis.steplogis)
# save the new model
#save(EDPredictorsLowAcuityPC.2016.logis.steplogis, file="EDPredictors.2016.logis.steplogis.Rda")

EDPredictorsLowAcuityPC.2017.df <- select(EDLowAcuityPC_cy2017.df, High.ED.Utilization.Indicator, Age.Group:Prior.ED.Visit.Count, Gender, Major.Region, Race.and.Ethnicity.Group, ED.Shift,  Season)
# save the 2017 validation data set
# save(ICUPredictorsLowAcuity.PC.JanOct2017.df, file="ICUPredictors.PC.JanOct2017.df.Rda")

## Predict outcomes by feeding the validation predictor variables into the model
EDPredictorsLowAcuityPC.t2016.v2017.steplogis.predict <- predict(EDPredictorsLowAcuityPC.2016.logis.steplogis, newdata=EDPredictorsLowAcuityPC.2017.df, type="response", se.fit=TRUE)

## Tally up the predicted outcomes vs. the actual outcomes included in the validation dataset (This is called scoring.)

evaluation.scores <- prediction(EDPredictorsLowAcuityPC.t2016.v2017.steplogis.predict$fit, EDPredictorsLowAcuityPC.2017.df$High.ED.Utilization.Indicator)
...
title( main="ROC Curve for Predictions of High ED Use with Low Acuity in Primary Care Well Child Patients",
...
title(main="True Positive and True Negative Rate Plots by Cutoff\nFor Predictions of High ED Use with Low Acuity in Priamry Care Well Child Patients",
      sub="Logistic model trained with CY2016 and validated with 2017 data")
...
title(main="Positive Predictive Value and Negative Predictive Value Plots by Cutoff\nFor Predictions of High ED Use with Low Acuity in Primary Care Well Child Patients",
      sub="Logistic model trained with CY2016 and validated with 2017 data")
