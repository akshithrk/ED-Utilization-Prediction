...
# 2016 ED Utilization
EDLowusePCLab_cy2016.df <- read.xlsx("High ED Use Low Use With Lab in 2016 Primary Care Clinic Well Child 2015-2016.xlsx")

# Validation data set
EDLowusePCLab_cy2017.df <- read.xlsx("High ED Use Low Use With Lab in 2017 Primary Care Clinic Well Child 2016-2017.xlsx")

# Fit model

## Select from the full dataset just the variables to try fitting.

EDPredictorsLowusePCLab.2016.df <- select(EDLowusePCLab_cy2016.df, High.ED.Utilization.3.Indicator, Age.Group:Prior.ED.Visit.Count.Orig, Gender, Major.Region, Race.and.Ethnicity.Group, ED.Shift)

## Fit a logistic regression model with all of the initial set of selected variables
EDPredictorsLowusePCLab.2016.logis <- glm(High.ED.Utilization.3.Indicator ~ ., family=binomial(logit), data=EDPredictorsLowusePCLab.2016.df)
summary(EDPredictorsLowusePCLab.2016.logis)
...
EDPredictorsLowusePCLab.2016.logis.steplogis <- stepAIC(EDPredictorsLowusePCLab.2016.logis, trace=0)
summary(EDPredictorsLowusePCLab.2016.logis.steplogis)
# save the new model
save(EDPredictorsLowusePCLab.2016.logis.steplogis, file="EDPredictorsLowusePCLab.2016.logis.steplogis.Rda")

EDPredictorsLowusePCLab.2017.df <- select(EDLowusePCLab_cy2017.df, High.ED.Utilization.3.Indicator, Age.Group:Prior.ED.Visit.Count.Orig, Gender, Major.Region, Race.and.Ethnicity.Group, ED.Shift)
# save the 2017 validation data set
save(EDPredictorsLowusePCLab.2017.df, file="EDPredictorsLowusePC2017Lab.df.Rda")

## Predict outcomes by feeding the validation predictor variables into the model
EDPredictorsLowusePCLab.t2016.v2017.steplogis.predict <- predict(EDPredictorsLowusePCLab.2016.logis.steplogis, newdata=EDPredictorsLowusePCLab.2017.df, type="response", se.fit=TRUE)

## Tally up the predicted outcomes vs. the actual outcomes included in the validation dataset (This is called scoring.)

evaluation.scores <- prediction(EDPredictorsLowusePCLab.t2016.v2017.steplogis.predict$fit, EDPredictorsLowusePCLab.2017.df$High.ED.Utilization.3.Indicator)
...
title( main="ROC Curve for Predictions of High ED Use with Low Resource with Certain Labs Use in Primary Care Well Child Patients",
...
title(main="True Positive and True Negative Rate Plots by Cutoff\nFor Predictions of High ED Use with Low Resource with Certain Labs Use in Primary Care Well Child Patients",
      sub="Logistic model trained with CY2016 and validated with 2017 data")
...
title(main="Positive Predictive Value and Negative Predictive Value Plots by Cutoff\nFor Predictions of High ED Use with Low Resource Use with Certain Labs in Primary Care Well Child Patients",
      sub="Logistic model trained with CY2016 and validated with 2017 data")
