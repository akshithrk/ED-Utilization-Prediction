...
#
#RVAR predicted.fit -output -numeric -vector  #Metric Expression: RScript<_RScriptFile="_RScriptFile="\\BI_isd\BI_Share\Ashley\ED Utilization Prediction\ED Utilization Low Resource V2.R", _InputNames="Age.Group, Problem.Count, CCC.Congenital, CCC.Hem.Imm, CCC.Malignancy, CCC.Metabolic, CCC.Renal, Public.or.Private.Metric, Holiday.Indicator, Cerebral.Palsy.DX,  Developmental.Disorders.Group.DX, Fractures.DX, Nausea.and.Vomiting.DX, Last.Primary.Care.Specialty.Seen, Prior.ED.Visit.Count.Orig, Gender, Major.Region,  Race.and.Ethnicity.Group", _WorkingDir="\\BI_ISD\BI_Share\Ashley\ED Utilization Prediction">(Age.Group, Problem.Count, CCC.Congenital, CCC.Hem.Imm, CCC.Malignancy, CCC.Metabolic, CCC.Renal, Public.or.Private.Metric, Holiday.Indicator, Cerebral.Palsy.DX,  Developmental.Disorders.Group.DX, Fractures.DX, Nausea.and.Vomiting.DX, Last.Primary.Care.Specialty.Seen, Prior.ED.Visit.Count.Orig, Gender, Major.Region,  Race.and.Ethnicity.Group)
#RVAR predicted.se.fit -output -numeric -vector  #Metric Expression: RScript<_RScriptFile="_RScriptFile="\\BI_isd\BI_Share\Ashley\ED Utilization Prediction\ED Utilization Low Resource V2.R", _InputNames="Age.Group, Problem.Count, CCC.Congenital, CCC.Hem.Imm, CCC.Malignancy, CCC.Metabolic, CCC.Renal, Public.or.Private.Metric, Holiday.Indicator, Cerebral.Palsy.DX,  Developmental.Disorders.Group.DX, Fractures.DX, Nausea.and.Vomiting.DX, Last.Primary.Care.Specialty.Seen, Prior.ED.Visit.Count.Orig, Gender, Major.Region,  Race.and.Ethnicity.Group", _WorkingDir="\\BI_ISD\BI_Share\Ashley\ED Utilization Prediction">(Age.Group, Problem.Count, CCC.Congenital, CCC.Hem.Imm, CCC.Malignancy, CCC.Metabolic, CCC.Renal, Public.or.Private.Metric, Holiday.Indicator, Cerebral.Palsy.DX,  Developmental.Disorders.Group.DX, Fractures.DX, Nausea.and.Vomiting.DX, Last.Primary.Care.Specialty.Seen, Prior.ED.Visit.Count.Orig, Gender, Major.Region,  Race.and.Ethnicity.Group)
if(exists("mstr.WorkingDir")) setwd(mstr.WorkingDir)  #Working Directory if executed by MicroStrategy
#
#MICROSTRATEGY_END

if(exists("mstr.WorkingDir")) setwd(mstr.WorkingDir)  #Working Directory if executed by MicroStrategy
## Rscript for predicting 
mstr.ErrMsg <- tryCatch({                                      #tryCatch for Exception Handling
  ...
  if(exists("mstr.ExFlag")) {
    ...
    load("EDPredictorsPC.2016.logis.steplogis.Rda") 
    predictors.df <- EDPredictorsLowusePC2017.df.Rda
  }
  #Make Predictions
  ...
  predicted.values <- predict(EDPredictorsPC.2016.logis.steplogis.Rda, newdata=predictors.df, type="response", se.fit=TRUE) #EDPredictorsPC.2016.logis.steplogis
