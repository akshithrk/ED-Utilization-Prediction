...
#
#RVAR predicted.fit -output -numeric -vector  #Metric Expression: RScript<_RScriptFile="\\bi_isd\BI_Share\Ashley\ED Utilization Prediction\High ED Low Resource Pred.R", _InputNames="Age.Group, Problem.Count, CCC.Congenital, CCC.Hem.Imm, CCC.Malignancy, CCC.Metabolic, CCC.Renal, Public.or.Private.Metric, Holiday.Indicator, Cerebral.Palsy.DX,  Developmental.Disorders.Group.DX, Fractures.DX, Nausea.and.Vomiting.DX, Last.Primary.Care.Specialty.Seen, Prior.ED.Visit.Count.Orig, Gender, Major.Region,  Race.and.Ethnicity.Group", _WorkingDir="\\bi_isd\BI_Share\Ashley\ED Utilization Prediction\High ED Low Resource Pred.R">(Age.Group, Problem.Count, CCC.Congenital, CCC.Hem.Imm, CCC.Malignancy, CCC.Metabolic, CCC.Renal, Public.or.Private.Metric, Holiday.Indicator, Cerebral.Palsy.DX,  Developmental.Disorders.Group.DX, Fractures.DX, Nausea.and.Vomiting.DX, Last.Primary.Care.Specialty.Seen, Prior.ED.Visit.Count.Orig, Gender, Major.Region,  Race.and.Ethnicity.Group)
#RVAR predicted.se.fit -output -numeric -vector  #Metric Expression: RScript<_RScriptFile="\\bi_isd\BI_Share\Ashley\ED Utilization Prediction\High ED Low Resource Pred.R", _InputNames="Age.Group, Problem.Count, CCC.Congenital, CCC.Hem.Imm, CCC.Malignancy, CCC.Metabolic, CCC.Renal, Public.or.Private.Metric, Holiday.Indicator, Cerebral.Palsy.DX,  Developmental.Disorders.Group.DX, Fractures.DX, Nausea.and.Vomiting.DX, Last.Primary.Care.Specialty.Seen, Prior.ED.Visit.Count.Orig, Gender, Major.Region,  Race.and.Ethnicity.Group")
if(exists("mstr.WorkingDir")) setwd(mstr.WorkingDir)  #Working Directory if executed by MicroStrategy
#
#MICROSTRATEGY_END

if(exists("mstr.WorkingDir")) setwd(mstr.WorkingDir)  #Working Directory if executed by MicroStrategy
## Rscript for predicting High ED Visits based on logistic model trained on 2016 data
mstr.ErrMsg <- tryCatch({                                      #tryCatch for Exception Handling
  if(exists("mstr.WorkingDir")) setwd(mstr.WorkingDir)         #Working Directory if executed by MicroStrategy
  #Get the prediction model
  load("EDPredictorsLowusePC.2016.logis.steplogis.Rda")
  #Get the data
