...
# 2017 ED Utilization
EDLowuse_cy2016.df <- read.xlsx("High ED Use Low Use in 2016 PCL Well Child 2015-2016.xlsx")

# Validation data set
EDLowuse_cy2017.df <- read.xlsx("High ED Use Low Use in 2017 PCL Well Child 2016-2017.xlsx")

# Fit model

## Select from the full dataset just the variables to try fitting.

EDPredictorsLowuse.2016.df <- select(EDLowuse_cy2016.df, High.ED.Utilization.Indicator, Age.Group:Prior.ED.Visit.Count, Gender, Major.Region, Race.and.Ethnicity.Group, ED.Shift)

## Fit a logistic regression model with all of the initial set of selected variables
EDPredictorsLowuse.2016.logis <- glm(High.ED.Utilization.Indicator ~ ., family=binomial(logit), data=EDPredictorsLowuse.2016.df)
summary(EDPredictorsLowuse.2016.logis)
...
EDPredictorsLowuse.2016.logis.steplogis <- stepAIC(EDPredictorsLowuse.2016.logis, trace=0)
summary(EDPredictorsLowuse.2016.logis.steplogis)
# save the new model
#save(EDPredictorsLowuse.2016.logis.steplogis, file="EDPredictors.2016.logis.steplogis.Rda")

EDPredictorsLowuse.2017.df <- select(EDLowuse_cy2017.df, High.ED.Utilization.Indicator, Age.Group:Prior.ED.Visit.Count, Gender, Major.Region, Race.and.Ethnicity.Group, ED.Shift)
# save the 2017 validation data set
# save(ICUPredictorsLowuse.7N.JanOct2017.df, file="ICUPredictors.7N.JanOct2017.df.Rda")

## Predict outcomes by feeding the validation predictor variables into the model
EDPredictorsLowuse.t2016.v2017.steplogis.predict <- predict(EDPredictorsLowuse.2016.logis.steplogis, newdata=EDPredictorsLowuse.2017.df, type="response", se.fit=TRUE)

## Tally up the predicted outcomes vs. the actual outcomes included in the validation dataset (This is called scoring.)

evaluation.scores <- prediction(EDPredictorsLowuse.t2016.v2017.steplogis.predict$fit, EDPredictorsLowuse.2017.df$High.ED.Utilization.Indicator)
...
title( main="ROC Curve for Predictions of High ED Use with Low Activity in PCL Well Child Patients",
...
title(main="True Positive and True Negative Rate Plots by Cutoff\nFor Predictions of High ED Use with Low Activity in PCL Well Child Patients",
      sub="Logistic model trained with CY2016 and validated with 2017 data")
